...
deleted by REST call, if the project was created incorrectly or by mistake.
	...
	is created, user has that amount of time in hours, specified in configuration,
	to delete the project using REST call, if project does not have created changes.
	After that time access permissions are applied and standard delete process is used
	to delete project.
