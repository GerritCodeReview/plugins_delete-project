...
in a single scheduled task execution.
  ...
  When `deleteTrashFolder` scheduling is enabled, the plugin scans
  repositories in batches rather than evaluating all folders in one
  long-running pass. Each batch is submitted as a separate recurring
  task, which helps distribute load over time and reduces the risk
  of blocking the WorkQueue for extended periods.
