...
(function(window) {
  ...
  var ELEMENT = '/static/repo-command-delete-repo.html';

  // PolyGerrit function

  /**
   * Create an empty DeleteRepoPlugin instance.
   *
   ...
   *
   */
  function DeleteRepoPlugin(gerritPlugin) {
    this._gerritPlugin = gerritPlugin;
    this._element = null;
  }

  DeleteRepoPlugin.prototype.handleRepo = function() {
    return this.handleRepoCommand();
  }

  /** @return {Promise} Resolves when the element is loaded. */
  DeleteRepoPlugin.prototype.loadElement = function() {
  ...
  DeleteRepoPlugin.prototype.handleRepoCommand = function() {
  ...
  window.Gerrit.install(function(self) {
      ...
      if (window.Polymer) {
        // Low-level API
        var plugin = new DeleteRepoPlugin(self);
        plugin.handleRepo();
        self.registerCustomComponent(
            'repo-command', 'repo-command-delete-repo');
      } else {
  ...
  if (window.Polymer) {
    window.__DeleteRepoPlugin = window.__DeleteRepoPlugin || DeleteRepoPlugin;
  }
})(window);
