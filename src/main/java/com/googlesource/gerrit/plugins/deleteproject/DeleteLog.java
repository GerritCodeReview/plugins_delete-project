...
class DeleteLog extends PluginLogFile {
  ...
  public DeleteLog(
    ...
    this.log = (Logger) LogManager.getLogger(DELETE_LOG_NAME);
  }
  ...
  public void onDelete(
    ...
    contextData.putValue(ACCOUNT_ID, user.getAccountId().toString());
    user.getUserName().ifPresent(u -> contextData.putValue(USER_NAME, u));
    contextData.putValue(PROJECT_NAME, project.get());
    if (options != null) {
      contextData.putValue(OPTIONS, OutputFormat.JSON_COMPACT.newGson().toJson(options));
    }
    if (ex != null) {
      contextData.putValue(ERROR, ex.toString());
    }

    Log4jLogEvent event =
        Log4jLogEvent.newBuilder()
            .setLoggerName(log.getName())
    ...
    log.getContext().getConfiguration().getLoggerConfig(log.getName()).log(event);
