...
public class Configuration {
  ...
  private static final long DEFAULT_DELETE_INTERVAL_DURATION_HOURS = 12;
  ...
  public long gettDeleteProjectTimeDurationHours() {
  ...
  private long getProjectDeleteDurationFromConfig(String configValue) {
    try {
      return ConfigUtil.getTimeUnit(configValue, DEFAULT_DELETE_INTERVAL_DURATION_HOURS, HOURS);
    } catch (IllegalArgumentException e) {
      log.atWarning().log(
          "The configured project delete duration is not valid: %s; using default value: %h hours",
          e.getMessage(), DEFAULT_DELETE_INTERVAL_DURATION_HOURS);
      return DEFAULT_DELETE_INTERVAL_DURATION_HOURS;
    }
  }
}
