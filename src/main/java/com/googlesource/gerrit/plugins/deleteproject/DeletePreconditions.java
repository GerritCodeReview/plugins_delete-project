...
class DeletePreconditions {
  ...
  void assertDeletePermission(ProjectResource rsrc) throws AuthException {
    if (!canDeleteByTimeCreation(rsrc) && !canDelete(rsrc)) {
  ...
  protected boolean canDeleteByTimeCreation(ProjectResource rsrc) {
    return canBeDeletedByTimeCreation(rsrc) && isRepoOwner(rsrc);
  }
