...
class DeletePreconditions {
  ...
  private boolean canBeDeletedByTimeDuration(ProjectResource rsrc) {
    try (Git git = new Git(repoManager.openRepository(rsrc.getNameKey()))) {
      try {
        ...
        if (LocalDateTime.ofInstant(
                Instant.ofEpochSecond(commitTimesSet.first()), ZoneId.systemDefault())
  ...
  protected boolean canDelete(ProjectResource rsrc) {
    if (canBeDeletedByTimeDuration(rsrc) && isRepoOwner(rsrc)) {
      return true;
    }
  ...
  protected boolean isRepoOwner(ProjectResource rsrc) {
