...
public final class DeleteProject {
  public interface Factory {
    DeleteProject create(@Assisted Project project,
        @Assisted("confirmed") boolean confirmed,
  ...
  @Inject
  public DeleteProject(ProjectCache projectCache,
  ...
  public void run() throws DeleteProjectFailedException, Exception {
    final String projectName = project.getName();

    // Don't let people delete All-Projects, that's stupid
    if (projectName.endsWith(AllProjectsNameProvider.DEFAULT)) {
      throw new DeleteProjectFailedException("Perhaps you meant to rm -fR " + site.site_path);
    }

    try {
      databaseDeleteHandler.assertCanDelete(project);
    } catch (Exception e) {
      throw new DeleteProjectFailedException("Cannot delete project " + projectName + ": "
          + e.getMessage());
    }
    ...
    if (!confirmed) {
      ...
      throw new DeleteProjectFailedException(msgBuilder.toString());
    }
  ...
  private void deleteFromDisk(Project project) throws IOException,
      RepositoryNotFoundException, DeleteProjectFailedException {
    // Remove from the jgit cache
    final Repository repository = repoManager.openRepository(project
        .getNameKey());
    if (repository == null) {
      throw new DeleteProjectFailedException("There was an error finding the project.");
    }
    ...
    if (!recursiveDelete(repository.getDirectory())) {
      throw new DeleteProjectFailedException("Error trying to delete "
