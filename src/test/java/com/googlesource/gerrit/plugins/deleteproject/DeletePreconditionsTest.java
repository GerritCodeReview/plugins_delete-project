...
public class DeletePreconditionsTest {
  ...
  @Test
  public void testCanDeleteIsTrueIfUserIsAdmin() {
  ...
  @Test
  public void testCanDeleteIsTrueIfUserHasDeletePermission() {
  ...
  @Test
  public void testCanDeleteIsTrueIfUserIsOwnerAndHasDeleteOwnPermission() {
  ...
  @Test
  public void testUserCannotDeleteIfCanDeleteAfterCreationAndCanDeleteAreFalse()
      throws IOException {
  ...
  @Test
  public void testCanDeleteAfterCreationIsTrue() throws GitAPIException, IOException {
    try (Repository repo = inMemoryRepositoryManager.openRepository(PROJECT_NAMEKEY)) {
      when(repoManager.openRepository(PROJECT_NAMEKEY)).thenReturn(repo);
      when(userPermission.testOrFalse(new PluginPermission(PLUGIN_NAME, DELETE_BY_CREATION_TIME)))
          .thenReturn(true);
      when(groupMembership.containsAnyOf(projectOwners)).thenReturn(true);
      when(config.getDeleteProjectTimeDuration()).thenReturn(10L);
      assertThat(preConditions.canDeleteAfterCreation(rsrc)).isTrue();
    }
  }

  @Test
  public void testCanDeleteAfterCreationIsFalseWhenUserDoesNotHaveCapability()
      throws GitAPIException, IOException {
    when(userPermission.testOrFalse(new PluginPermission(PLUGIN_NAME, DELETE_BY_CREATION_TIME)))
        .thenReturn(false);
    assertThat(preConditions.canDeleteAfterCreation(rsrc)).isFalse();
  }

  @Test
  public void testCanDeleteAfterCreationIsFalseWhenUserIsNotRepoOwner()
      throws GitAPIException, IOException {
    when(userPermission.testOrFalse(new PluginPermission(PLUGIN_NAME, DELETE_BY_CREATION_TIME)))
        .thenReturn(true);
  ...
  @Test
  public void testCanDeleteAfterCreationIsFalseWhenDurationForDeleteEnds()
      throws GitAPIException, IOException {
    try (Repository repo = inMemoryRepositoryManager.openRepository(PROJECT_NAMEKEY)) {
      when(repoManager.openRepository(PROJECT_NAMEKEY)).thenReturn(repo);
      when(userPermission.testOrFalse(new PluginPermission(PLUGIN_NAME, DELETE_BY_CREATION_TIME)))
